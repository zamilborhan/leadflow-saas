...
import { checkWhatsAppHealth } from "@/src/lib/integrations/whatsapp/service";
...
/**
 * Live messaging-health check against Meta. Requires businesses.update
 * (OWNER/ADMIN). Persists the overall status + check time for display.
 */
export async function POST(
  ...
  try {
    ...
    const health = await checkWhatsAppHealth(g.context);
