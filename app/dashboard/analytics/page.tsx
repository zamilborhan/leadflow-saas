  ...
  if (total === 0) {
    return (
      <EmptyState
        ...
        description="Widen the date range or clear the funnel filters."
      />
    );
  }
  return (
    <div
      role="img"
      aria-label={`Leads per day, ${total} total across ${buckets.length} days`}
...
/** Horizontal funnel bars with counts + per-slice conversion rates. */
      ...
      {slices.slice(0, 8).map((s) => (
        <li key={s.key}>
          <div className="flex items-baseline justify-between gap-3 text-sm">
            <span className="truncate font-medium text-slate-800" title={s.key}>
              {s.key}
            </span>
            <span className="shrink-0 text-xs text-slate-500">
              {s.count} leads · {formatPct(s.conversionRate)} converted
            </span>
          </div>
...
/**
 * Workspace analytics, scoped to the current business (?businessId= or the
 * user's first workspace). Every metric derives from tenant-scoped reads;
 * archived leads are excluded throughout.
 */
export default async function AnalyticsPage({
  ...
  if (businesses.length === 0) {
    return (
      <div className="flex flex-col gap-6">
        ...
        <Card>
          <EmptyState title="No workspace yet" description="Create a workspace to start measuring your funnel." />
        </Card>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Analytics"
        description={`Funnel metrics for ${business.name} · ${rangeLabel}.`}
        eyebrow="Analytics"
      />
      ...
      <section aria-label="Key metrics" className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
        ...
        <StatCard
          ...
          value={formatPct(analytics.kpis.conversionRate)}
          hint={analytics.kpis.conversionRate === null ? "No leads in range" : "Converted ÷ total leads"}
        />
        <StatCard
          label="Follow-up completion"
          value={formatPct(analytics.kpis.followUpCompletion.completionRate)}
          hint={
            analytics.kpis.followUpCompletion.total === 0
              ? "No follow-ups in range"
              : `${analytics.kpis.followUpCompletion.completed} of ${analytics.kpis.followUpCompletion.total} completed`
          }
        />
        <StatCard
          label="Pending follow-ups"
          value={String(analytics.kpis.followUpCompletion.pending)}
          hint="Scheduled or overdue, not completed"
        />
        <StatCard
          label="Overdue follow-ups"
          value={String(analytics.kpis.overdueFollowUps)}
          hint={analytics.kpis.overdueFollowUps === 0 ? "Nothing overdue. Nice work." : "Past due and still open"}
        />
      </section>
      ...
      <section aria-label="Attribution" className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <FunnelBars slices={analytics.leadsBySource} emptyHint="No sources in this range." />
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <FunnelBars slices={analytics.leadsByCampaign} emptyHint="No campaigns in this range." />
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <FunnelBars slices={analytics.leadsByAdSet} emptyHint="No ad sets in this range." />
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            <FunnelBars slices={analytics.leadsByAd} emptyHint="No ads in this range." />
          </CardContent>
        </Card>
      </section>
      ...
      <Card>
        ...
        {analytics.agents.length === 0 ? (
          <EmptyState
            ...
            description="Assign leads to see per-agent performance."
          />
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {analytics.agents.map((a) => (
                <TableRow key={a.userId}>
                  ...
                  <TableCell>
                    {a.followUpsCompleted} of {a.followUpsTotal}
                  </TableCell>
